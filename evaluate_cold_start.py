...
def main():
    ...
    warm_hits = defaultdict(int); cold_hits = defaultdict(int)
    warm_ndcg = defaultdict(float); cold_ndcg = defaultdict(float)
    total_warm = 0; total_cold = 0

    for uid, data in tqdm(test_seqs.items(), desc="Evaluating users (low‑exposure cold split)"):
        history = data['history']
        target_asin = data['target_asin']
        count = raw_pop.get(target_asin, 0)
        is_cold = (count <= args.cold_threshold)
    ...
    print(f"\nLow‑Exposure Cold‑Start Evaluation (threshold ≤{args.cold_threshold})")
    print(f"Warm items: {total_warm}, Cold items: {total_cold}")
    if total_warm > 0:
        print("\nWarm Item Performance:")
        for k in [1, 5, 10]:
            print(f"HR@{k}: {warm_hits[k]/total_warm:.4f}  NDCG@{k}: {warm_ndcg[k]/total_warm:.4f}")
    if total_cold > 0:
        print("\nCold Item Performance:")
        for k in [1, 5, 10]:
            print(f"HR@{k}: {cold_hits[k]/total_cold:.4f}  NDCG@{k}: {cold_ndcg[k]/total_cold:.4f}")
    else:
        print("No cold items with this threshold.")

...
def build_item_index(model, meta_dict, device, batch_size, max_seq_length, tokenizer):
    model.eval()
...
def train(args):
    ...
    print(f"CPT sampler ready: {len(pair_weights)} pairs, weight range [{min(pair_weights):.6f}, {max(pair_weights):.2f}]")
    ...
    # ---- Item index + GPU embeddings ----
    torch.cuda.empty_cache(); gc.collect()
    item_index, item_asins, item_embs = build_item_index(
        model, meta_dict, device, batch_size=32, max_seq_length=args.max_seq_length, tokenizer=tokenizer)
    torch.cuda.empty_cache(); gc.collect()
    ...
    for epoch in range(cpt_epochs):
        ...
        if val_seqs is not None:
            ...
            print(f"CPT Epoch {epoch+1} → HR@10: {hr:.4f}  NDCG@10: {ndcg:.4f}")
